package com.why.library.common.tablayout;
...
import com.why.library.common.tablayout.listener.WOnTabSelectListener;
import com.why.library.common.tablayout.utils.WFragmentChangeManager;
import com.why.library.common.tablayout.utils.WUnreadMsgUtils;
import com.why.library.common.tablayout.widget.WMsgView;
 ...
 * @author  WangHuanyu
 * @todo 没有继承HorizontalScrollView不能滑动,对于ViewPager无依赖
 * @date 2016/9/19 13:21
 */
public class WCommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    private Context mContext;
    private ArrayList<WCustomTabEntity> mTabEntitys = new ArrayList<>();
    ...
    private WFragmentChangeManager mFragmentChangeManager;

    public WCommonTabLayout(Context context) {
        this(context, null, 0);
    }

    public WCommonTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public WCommonTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setTabData(ArrayList<WCustomTabEntity> tabEntitys) {
    ...
    /** 关联数据支持同时切换fragments */
    public void setTabData(ArrayList<WCustomTabEntity> tabEntitys, FragmentActivity fa, int containerViewId, ArrayList<Fragment> fragments) {
        mFragmentChangeManager = new WFragmentChangeManager(fa.getSupportFragmentManager(), containerViewId, fragments);
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            if (mIconGravity == Gravity.LEFT) {
                tabView = View.inflate(mContext, R.layout.f_layout_tab_left, null);
            } else if (mIconGravity == Gravity.RIGHT) {
                tabView = View.inflate(mContext, R.layout.f_layout_tab_right, null);
            } else if (mIconGravity == Gravity.BOTTOM) {
                tabView = View.inflate(mContext, R.layout.f_layout_tab_bottom, null);
            } else {
                tabView = View.inflate(mContext, R.layout.f_layout_tab_top, null);
            }
...
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
            ...
            if (mIconVisible) {
                ...
                WCustomTabEntity tabEntity = mTabEntitys.get(i);
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    public void showMsg(int position, int num) {
        ...
        WMsgView tipView = (WMsgView) tabView.findViewById(R.id.rtv_msg_tip);
        if (tipView != null) {
            WUnreadMsgUtils.show(tipView, num);
    ...
    public WMsgView getMsgView(int position) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        View tabView = mTabsContainer.getChildAt(position);
    ...
    private WOnTabSelectListener mListener;

    public void setOnTabSelectListener(WOnTabSelectListener listener) {
